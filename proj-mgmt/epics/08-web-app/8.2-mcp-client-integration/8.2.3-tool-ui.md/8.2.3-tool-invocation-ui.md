...
- [x] Create a `Tools.razor` page at `/tools`
- [x] On page load, call `IMcpClientService.ListToolsAsync()` and display tools in a searchable/filterable list (card layout)
- [x] Each tool card shows: name, description, parameter schema
- [x] Clicking a tool opens a detail panel with a dynamically generated form:
...
- [x] "Invoke" button calls `IMcpClientService.CallToolAsync(toolName, arguments)`
- [x] Display the result in a formatted panel:
  - Text content rendered as monospace
  - JSON content rendered with pretty-printing
  - Errors displayed with the MCP error message
- [x] Show a loading indicator while the tool call is in progress
- [x] History panel: list of recent tool invocations in the current session (tool name, timestamp, success/failure)
...
| ID | Title | Status |
|---|---|---|
| [BUG-002](./BUG-002-tools-page-404/BUG-002-tools-page-404.md) | Tools page returns 404 — MCP endpoint path mismatch | FIXED |
