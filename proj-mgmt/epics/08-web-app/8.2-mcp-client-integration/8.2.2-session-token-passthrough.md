...
- [x] Inject `ITokenAcquisition` (from `Microsoft.Identity.Web`) into `McpClientService`
- [x] Before each MCP server request, acquire the token: `await tokenAcquisition.GetAccessTokenForUserAsync(new[] { "api://<client-id>/access_as_user" })`
- [x] Set the token on the `HttpClient` used by `HttpClientTransport`:
  ```csharp
  httpClient.DefaultRequestHeaders.Authorization =
      new AuthenticationHeaderValue("Bearer", token);
  ```
- [x] Handle `MsalUiRequiredException` — if the token cache is empty or consent is needed, redirect the user to re-authenticate (Blazor Server can trigger a redirect via `NavigationManager`)
  - Implemented in `Tools.razor`: catches `MicrosoftIdentityWebChallengeUserException` and redirects to `MicrosoftIdentity/Account/SignIn` with `forceLoad: true`
- [x] Verify that each user session gets its own `HttpClient` instance (no shared static client that leaks tokens across sessions)
  - `McpClientService` is registered as scoped; each Blazor circuit gets its own instance
