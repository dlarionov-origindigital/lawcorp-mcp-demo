...
- [x] When `UseAuth=true`, clicking "Sign in" redirects to Entra ID and returns with a valid session
- [x] After sign-in, the home page displays the user's display name, role, and email from JWT claims
- [x] The header `LoginDisplay` shows a persona avatar, name, role badge, and "Sign out" button
- [x] Sign-out clears the session and redirects through Entra ID sign-out
- [x] `/account/claims` page shows all JWT claims for debugging token contents
- [x] When `UseAuth=false`, all pages load without error; header shows "Demo mode" badge; no dead links
- [x] `appsettings.json` (committed) contains empty placeholders; real values are in gitignored `appsettings.Development.json`
- [x] `appsettings.Development.json.example` documents all required web app auth settings
