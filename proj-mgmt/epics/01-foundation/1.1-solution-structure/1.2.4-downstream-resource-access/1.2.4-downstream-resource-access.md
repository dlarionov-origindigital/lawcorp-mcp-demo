...
Three distinct patterns are used, all starting from the same inbound bearer token:
...
| External API (document service) | OBO token exchange | `IDownstreamTokenProvider` acquires an external-API-scoped token via OBO, then HTTP calls use that token. See [Feature 1.4](../../1.4-external-api/1.4-external-api.md). |
...
> **External API downstream access** is tracked separately under [Feature 1.4](../../1.4-external-api/1.4-external-api.md) (stories 1.4.3 and 1.4.4). This story covers Graph and local DB access patterns.
...
- [ ] MCP tools that access the external API use OBO to delegate the user's identity (see [1.4.4](../../1.4-external-api/1.4.4-mcp-server-obo-to-external-api.md))
- [ ] A Partner persona can access resources that an Intern persona cannot, end-to-end
- [ ] Consent errors from Graph and the external API are surfaced as structured MCP error responses (not unhandled exceptions)
