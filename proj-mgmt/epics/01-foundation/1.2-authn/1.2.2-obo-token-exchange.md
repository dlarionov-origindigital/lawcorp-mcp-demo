...
## Expanded Scope (Multi-Downstream OBO)

The original implementation targets Microsoft Graph only. With the addition of [Feature 1.4: External Downstream API](../1.4-external-api/1.4-external-api.md), the OBO infrastructure must support **multiple downstream APIs** with distinct `api://` scopes. See [1.4.4](../1.4-external-api/1.4.4-mcp-server-obo-to-external-api.md) for the external API OBO story.

- [ ] `IDownstreamTokenProvider` supports parameterized downstream API selection (e.g., `GetTokenAsync("ExternalApi")` vs. `GetTokenAsync("MicrosoftGraph")`)
- [ ] `DownstreamApis:ExternalApi` configuration section supported alongside `DownstreamApis:MicrosoftGraph`
- [ ] Token caching works for all downstream API tokens independently

## Notes

Depends on 1.2.1 (Entra ID auth middleware) being complete.
Consumed by [1.2.4.1](../1.1-solution-structure/1.2.4-downstream-resource-access/1.2.4.1-graph-client-obo-provider.md) (Graph client with OBO provider) and [1.4.4](../1.4-external-api/1.4.4-mcp-server-obo-to-external-api.md) (External API OBO).
