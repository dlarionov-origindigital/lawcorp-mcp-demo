...
**Goal:** Verify that the authorization model from PRD Section 4.2 is fully enforced: role-based access denies the right personas, row-level security prevents cross-scope data leakage, and field-level redaction strips privileged content for restricted roles.
...
Depends on: 7.1 (test infrastructure), 7.2 (persona fixture), 1.3.1–1.3.3 (auth handlers)
