...
{
    [McpServerTool(Name = McpToolName.Intake.CreateRequest), Description("Create a new client intake request for a potential new client or matter.")]
    ...
    [McpServerTool(Name = McpToolName.Intake.GetRequest), Description("Retrieve the details and current status of an intake request.")]
    ...
    [McpServerTool(Name = McpToolName.Intake.RunConflictCheck), Description("Run conflict-of-interest checks for an intake request against existing firm relationships.")]
    ...
    [McpServerTool(Name = McpToolName.Intake.Approve), Description("Approve or reject an intake request and optionally assign a partner. Partner-only action.")]
    ...
    [McpServerTool(Name = McpToolName.Intake.GenerateEngagementLetter), Description("Generate an engagement letter document from an approved intake request.")]
