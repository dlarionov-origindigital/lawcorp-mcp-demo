...
{
    [McpServerTool(Name = McpToolName.Documents.Search), Description("Search documents by keyword, type, case, and author across the document management system.")]
    ...
    [McpServerTool(Name = McpToolName.Documents.Get), Description("Retrieve a document's metadata and full content by document ID.")]
    ...
    [McpServerTool(Name = McpToolName.Documents.Draft), Description("Generate a draft document from a standard template, populated with case and client data.")]
    ...
    [McpServerTool(Name = McpToolName.Documents.UpdateStatus), Description("Update the status of a document. Valid transitions: Draft → UnderReview → Final → Filed.")]
    ...
    [McpServerTool(Name = McpToolName.Documents.ListByCase), Description("List all documents associated with a case, with optional type and status filters.")]
