...
public class DocumentGenerator(Random rng)
{
    ...
    public Document Generate(Case @case, Attorney author)
    {
    ...
    public IEnumerable<Document> GenerateForCase(Case @case, IList<Attorney> attorneys, int count = 3)
    {
        for (var i = 0; i < count; i++)
        {
            var author = attorneys[rng.Next(attorneys.Count)];
