...
public class CaseGenerator(Random rng)
{
    ...
    public Case Generate(IList<Client> clients, IList<PracticeGroup> practiceGroups, IList<User> attorneys)
    {
        ...
        var lead = attorneys
            .Where(a => a.PracticeGroupId == group.Id && a.FirmRole == FirmRole.Partner)
            .MinBy(_ => rng.Next())
            ?? attorneys.First(a => a.FirmRole == FirmRole.Partner);
    ...
    public IEnumerable<Case> GenerateMany(int count, IList<Client> clients, IList<PracticeGroup> groups, IList<User> attorneys)
    {
